...
function generateLogTableRow(id, summary) {
    ...
    var idCell = buildHTML('td', '<a href="https://wfirma.atlassian.net/browse/' + id + '" class="taskLink" style="margin:0;">' + id + "</a>", {class: 'issue-id'});
    ...
    var copyButton = buildButton("copy", id);
...
async function copyButtonClick(evt) {
    var issueId = $(evt.target).data('issue-id');
    var taskName = $('tr[data-row-issue-id=' + issueId + '] td.issue-summary').text();
    try {
...
function stopButtonClick(evt) {
    ...
    let save = window.confirm(text + " spent. \nDo you want to send update to Jira task?");
    if(save) {
        logTimeClick(evt);
    }
}
...
function buildButton(type, id) {
    ...
    if (type == "copy") {
        ...
        $button.on('click', copyButtonClick);
    }
    return $button;
}
...
function ProjectStatuesSuccess(data) {
    ...
    if ($.type(data) === "array" && data.length > 0) {
        var statuses = $.grep(data, function (e) {
            return e.name == "Task";
        })[0].statuses;
...
function updateIcon() {
    if (AnyTimerStarted())
        chrome.action.setIcon({path: "images/icon_run.png"});
    else
